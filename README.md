...
下図の真ん中あたりの「出荷作業支援」が今回のGASが支援する範囲。手作業をなるべく減らし、人間のミスや手戻りがなくなるよう、GASにがんばってもらいたい。
...
1. CSVの、フィールド１文字目がゼロの場合、Excelに取り込むとゼロが消える。手作業で変更するのではなく、自動でゼロが入るようになっていてほしい
2. 送り状CSVデータは、「未発送」のみが出力対象で、「未入金」は出力できない。未入金レコードも、送り状CSVに取り込みたい
    - STORESのCSV書き出し機能のデフォルト。オプション指定できない
    - 入金前に、送り状の作成に着手しておくことができない
    - 手作業でやるとミスが多発しそうで怖い
3. 送り状CSVデータの「依頼主」が固定値なので、手作業での書き換えが発生する。自動的に書き換わっていてほしい
    - 購入者と配送先が異なる場合、プレゼントと思われるので「送り主＝購入者」としたい
    - STORESの仕様で、「送り主」には必ずショップの住所・氏名が入っている
    - 手作業でやるとミスが多発しそうで怖い
4. いくつかの列には固定値を埋めたい
    - たとえば請求先コードは、すべてのレコードで同じ値
    - 手作業（Excelのコピペ）でもできるが、コンピュータが得意なことはコンピュータに任せたい
5. 出荷時の最終チェックに当たり、チェック用シートを自動的に出してほしい
